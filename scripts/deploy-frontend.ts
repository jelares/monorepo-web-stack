...
async function uploadDirectory(
  ...
  console.log(
    `Uploading ${files.length} files to s3://${bucketName}/${s3Prefix}/`,
  );
...
async function updateCloudFrontOrigin(
  distributionId: string,
  newOriginPath: string,
): Promise<void> {
  // Get current config
  const getResponse = await cloudfront.send(
...
async function cleanupOldVersions(
  ...
  if (versionsToDelete.length === 0) {
    console.log(
      `Keeping ${versions.length} versions (limit: ${versionsToKeep})`,
    );
    return;
  }
...
async function main() {
  ...
  if (!bucketName || !distributionId || !versionParamName) {
    console.error(
      "Error: Could not get stack outputs. Make sure infrastructure is deployed.",
    );
    process.exit(1);
  }
